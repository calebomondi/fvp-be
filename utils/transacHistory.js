...
export function analyzeTransactions(moralisResponse) {
  ...
  analysis.summary.dateRange.to = transactions[0].block_timestamp;
  analysis.summary.dateRange.from = transactions[transactions.length - 1].block_timestamp;
  ...
  transactions.forEach((tx, index) => {
    ...
    // Calculate gas fees
    const gasFee = parseFloat(tx.transaction_fee);
    ...
    const enhancedTx = {
      ...
      timestamp: txDate,
      gasFee: gasFee,
      usdValue: 0, // Could be enhanced with price data
    ...
    //analysis.categories[category].transactions.push(enhancedTx);
    ...
    if (tx.native_transfers && tx.native_transfers.length > 0) {
      tx.native_transfers.forEach(transfer => {
        const value = parseFloat(transfer.value_formatted);
  ...
  const gasFees = transactions.map(tx => parseFloat(tx.transaction_fee));
...
function analyzeBehavior(tx, index, allTransactions, analysis) {
  ...
  if (index > 0) {
    ...
    const prevTime = new Date(prevTx.block_timestamp);
  ...
  if (tx.category === 'token swap' && tx.erc20_transfers) {
    const swapValue = tx.erc20_transfers.reduce((sum, transfer) => 
      sum + Math.abs(parseFloat(transfer.value_formatted)), 0
    );
...
function calculateRiskScore(analysis) {
  ...
  if (analysis.gasFeeAnalysis.highest && 
      parseFloat(analysis.gasFeeAnalysis.highest.transaction_fee) > avgGasFee * 5) {
    score += 5;
  }
